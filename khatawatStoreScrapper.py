...
def get_product_details(soup):
    ...
    product_details = {
        ...
        "price": modal_content.find(id="offerModal___BV_modal_footer_").find(class_="color-primary-dark").text,
...
def scrape_products(url, max_pages=10000):
    ...
    no_new_products_attempts = 0  # Track number of attempts with no new products
    max_attempts = 5  # Maximum number of attempts before stopping
    ...
    while page <= max_pages:
        try:
            ...
            if current_num_products == prev_num_products:
                ...
                if no_new_products_attempts >= max_attempts:
                    break  # No more new products loaded after max attempts, exit loop
            else:
                no_new_products_attempts = 0  # Reset attempts counter
